(function( $ ) {
    ...
    $(document).ready(function () {
        ...
        const formInput = "form#kleverlist_sendy_wc_migration_settings :input";

        const storageKey = 'sendyMigrationProgress';
        ...
        $(document).on('submit', '#kleverlist_sendy_wc_migration_settings', function (e) {
            e.preventDefault();
            kleverlistSendyMigrationSettings();
        });

        // Function to update the progress bar and text
        function kleverlistSendyMigrationUpdateProgress() {
        ...
        function kleverlistSendyCheckProgressMigration() {
            $.ajax({
                url: ajaxurl,
                type: 'GET',
                data: {
                    action: 'kleverlist_sendy_check_progress_migration',
                    security: nonce,
                },
        ...
        function kleverlistSendyMigrationDeleteOption() {
            $.ajax({
                url: ajaxurl,
                type: 'post',
                data: {
                    action: 'kleverlist_sendy_reset_migration_progress',
                    security: nonce,
                },
                success: function (response) {
                    if (response.status) {
        ...
        function kleverlistSendyMigrationSettings() {
            let responseClass = '.kleverlist-migration-response';

            let sendy_list_id = $('#kleverlist_sendy_list').val();
            let preventContacts = ( $("#kleverlist_sendy_prevent_existing_contacts").prop('checked') == true ) ? 'yes' : 'no';
            ...
            if (sendy_list_id == '') {
                ...
                progressInterval = setInterval(kleverlistSendyMigrationUpdateProgress, 3000);

                $.ajax({
                    type: "POST",
                    url: ajaxurl,
                    data: {
                        action: 'kleverlist_sendy_migration_settings',
                        sendy_list_id: $('#kleverlist_sendy_list').val(),
